...
/datum/team/nuclear/New()
	..()
	syndicate_name = "Chaos Insurgency"
...
/datum/team/nuclear/roundend_report()
	...
	switch(get_result())
		if(NUKE_RESULT_FLUKE)
			...
			parts += "<B>The crew of [station_name()] gave [syndicate_name] operatives back their bomb! The chaos base was destroyed!</B> Next time, don't lose the nuke!"
		...
		if(NUKE_RESULT_WRONG_STATION)
			parts += "<span class='redtext big'>Foundation Minor Victory!</span>"
		...
		if(NUKE_RESULT_HIJACK_DISK)
			...
			parts += "<B>[syndicate_name] operatives failed to destroy [station_name()], but they managed to secure the disk and hijack the emergency shuttle, causing it to land on the chaos base. Good job?</B>"
		if(NUKE_RESULT_HIJACK_NO_DISK)
			parts += "<span class='greentext big'>Chaos Insurgency Insignificant Victory!</span>"
			parts += "<B>[syndicate_name] operatives failed to destroy [station_name()] or secure the disk, but they managed to hijack the emergency shuttle, causing it to land on the chaos base. Good job?</B>"
	...
	var/text = span_header("<br>The Chaos Insurgency operatives were:")
	var/purchases = ""
	var/TC_uses = 0
...
/datum/team/nuclear/proc/admin_spawn_reinforcement(mob/admin)
	...
	var/mob/chosen_one = SSpolling.poll_ghost_candidates(
		...
		role_name_text = "emergency chaos reinforcement",
		amount_to_pick = 1,
	)
